steal(
    ...
    'vitco_web/principal/principal.js').then('./views/init.ejs', function($){    // A este steal le agrego una pantalla ejs o algo asi
         ...
         {
             defaults : {
                 title: 'Visualizaci�n de transacciones',
                 error: ''
             }
         },
         {
             'init' : function(){ // inicializacion
                 this.element.html(can.view('./login/views/init.ejs',{message: this.options.error, img_url: url})); // La idea es mostrar el elemento login.ejs
